---
description: Resetting your password
title: Resetting Your Password
---

### Resetting your password


If you're having trouble logging in to your account or simply need to update your credentials, follow these quick steps to reset your password securely. This guide walks you through the entire process — from requesting a reset link to creating a new password.


If you need to reset the password, go to [www.platform.radix.com,](http://www.platform.radix.com) type your email address and click "Continue" 


<Frame caption="Login interface for Radix Analytics with email input and continue button.">
  <img src="/images/radix/dee5775b3096acd44f1874330c93943d41e7b782.png" alt="" />
</Frame>


### Selecting Forgot Password


On the next step click "Forgot Password"


<Frame caption="Password entry interface for Radix Analytics with options for login and password recovery.">
  <img src="/images/radix/c777074a76f1f996dd1df78fbaa6152664b940ed.png" alt="" />
</Frame>


### Reset Link


Type in your email address and click "Send password reset link"


<Frame caption="Password reset form with email input and instructions for Radix Analytics.">
  <img src="/images/radix/5a3bba33184230b8a8932fbaf8db70356e6ddc19.png" alt="" />
</Frame>


Select the "Click here to reset your password" link in the email you received.


<Frame caption="Password reset email from Radix with a clickable link.">
  <img src="/images/radix/0ea3d47d7b8c130b410ea1dbec9985fdb1fd5180.png" alt="" />
</Frame>


### Entering new password


 Type in the new password and click "Reset password"


<Frame caption="Password reset form with fields for new password and confirmation.">
  <img src="/images/radix/2378c1610abca8681535248356391a5a64e79f35.png" alt="" />
</Frame>


 


### Need Additional Help?


If you don’t receive the reset email within a few minutes, be sure to check your spam or junk folder. Still having trouble? Contact our Support Team for further assistance.